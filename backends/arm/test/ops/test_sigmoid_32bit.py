...
def test_int16_table_small_output_range_is_not_degenerate():
    ...
    (0 dB PTQ<->FVP). The table must remain a non-degenerate, monotonic ramp.
    """
    # qparams as observed for an ECAPA SE-block sigmoid (input spans ~+-22.4,
    ...
    # generate_16_bit_table_values does not use ``self``; call it unbound.
    lut, rescale_lshift = InsertTableOpsPass.generate_16_bit_table_values(
        None, torch.sigmoid, in_quantargs, out_quantargs
    )
